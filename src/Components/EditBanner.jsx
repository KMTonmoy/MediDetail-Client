...
const EditBanner = () => {
    ...
    const handleDelete = async (id) => {
        ...
        if (result.isConfirmed) {
            try {
                const response = await fetch(`https://mediserver.vercel.app/banners/${id}`, { method: 'DELETE' });
    ...
    const handleSave = async () => {
        ...
        try {
            const response = await fetch(`https://mediserver.vercel.app/banners/${_id}`, {
                method: 'PUT',
                headers: {
        ...
        <div className="relative z-0">
            {images.length === 0 ? (
                ...
                images[currentImageIndex] && (
                    <div
                        className="relative h-[800px] lg:h-[800px] transition-all duration-700 ease-in-out"
                        style={{
                            backgroundColor: images[currentImageIndex].bgColor,
                            backgroundImage: `linear-gradient(to right, ${images[currentImageIndex].bgColor}, transparent)`,
                        }}
                    >
                        <div className="container mx-auto grid lg:grid-cols-2 gap-5 items-center px-5 lg:px-10 h-full">
